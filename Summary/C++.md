...
* [C++和C动态库链接使用](http://www.cnblogs.com/China3S/p/3267113.html)或[C动态库供C++使用](https://blog.csdn.net/sjin_1314/article/details/20958149)
...
* 取非静态数据成员的地址返回的是相对与类首地址的偏移量+1，加1是为了区别一个没有指向任何数据成员的指针和第一个指向数据成员的指针。
   * 取静态成员函数的指针得到的都是内存中的真正地址，因为编译器已经将成员函数设置为非成员函数，对于静态成员函数也是一样。
   * 静态函数取类型是指向普通函数的指针，非静态成员函数取类型指向的是成员函数指针类型，主要原因是需要传入this指针。
   * 取虚函数的地址只是其索引值
 
* inline函数的临时性变量产生条件：
  * inline函数参数中有非常量表达式，需要临时变量记录该非常量表达式的值
  * inline函数以一个单一表达式作多重调用
* 纯虚函数：
  * 纯虚函数一般只是在抽象基类中声明，不会被定义，但是也可以被定义且调用，此时只能只能静态调用，即由抽象基类和作用域运算符调用
  * 如果定义了纯虚析构函数，那么必须定义该函数，因为派生类需要逐层向上调用上层的析沟函数，否则会出现链接错误
* NRV优化
* 构造函数的流程与析沟函数的流程：
  * 构造函数的流程：涉及到菱形继承时，必须到达最低的派生类时才可以调用虚基类的构造函数，不然会产生多个副本。实现方法是在构造函数中加入bool形参指示是否为最深的派生类P206
  * 析沟函数的流程：P234
* 在构造函数和析构函数中调用虚函数
  * 使用的都是构造函数和析构函数对应的对象类型的版本
...
* 数组中存储的是基类指针，但基类指针指向的是派生类，这种情况删除数组时默认调用的是基类类型对应的析构函数B* ptr=new D[10];按照B的析构函数析构
